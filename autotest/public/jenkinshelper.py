...
class Jenkins:
    def __init__(self, apk_path):
        ...
        if PROJECT== 'ENGAGE':
            if PRODUCT == 'smartenglish':

                self.Jenkins_build_url = JENKINS_HOST_ANDROID + "/view/Engage/job/engage-ec-testing/lastSuccessfulBuild/api/json"
                self.apk_url = "artifact/engage/build/outputs/apk/smartenglishUat/debug/"

            elif PRODUCT == 'englishtown' or PRODUCT == 'corporate':

                self.Jenkins_build_url = JENKINS_HOST_ANDROID + "/view/Engage/job/engage-android-snapshot/lastSuccessfulBuild/api/json"
                self.apk_url="artifact/engage/build/outputs/apk/"
    ...
    def get_build_url(self):
        ...
        try:
            builds_urls = requests.get(self.Jenkins_build_url,auth=('shirley.licp', '1'))

            builds = [each_build['relativePath'] for each_build in builds_urls.json()['artifacts']]
            preporter.info(builds)
            current_build = re.findall("engage-{}-{}-debug.*?.apk".format(PRODUCT,ENV),",".join(builds))


            preporter.info(current_build)

            if current_build:
                apk_url = builds_urls.json()['url'] + self.apk_url + current_build[0]
